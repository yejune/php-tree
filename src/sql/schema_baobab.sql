...
CREATE TABLE IF NOT EXISTS GENERIC (
        ...
        parent_id INTEGER UNSIGNED default '0' NOT NULL,
...
MODIFIES SQL DATA
    ...
    BEGIN
        ...
        DECLARE drop_parent_id INTEGER UNSIGNED;
        ...
        */
        ...
        SELECT tree_id, parent_id, id, lft, rgt
        INTO drop_tree_id, drop_parent_id, drop_id, drop_lft, drop_rgt
        FROM GENERIC
        WHERE id = node;
...
DETERMINISTIC
    ...
    BEGIN
        ...
        IF parent_id = 0 THEN /* inserting a new root node*/
                ...
                INSERT INTO GENERIC(tree_id, parent_id, id, lft, rgt)
                VALUES (cur_tree_id, parent_id, NULL, 1, num);
        ...
        ELSE /* append a new node as last right child of his parent */
                ...
                INSERT INTO GENERIC(tree_id, parent_id, id, lft, rgt)
                VALUES (cur_tree_id,parent_id, NULL, num, (num + 1));
    ...
    main:BEGIN
        ...
        ELSE
                BEGIN
                    ...
                    DECLARE choosen_parent_id INTEGER UNSIGNED;

                    START TRANSACTION;

                    SELECT tree_id,parent_id,rgt
                    INTO choosen_tree,choosen_parent_id,lft_sibling
                    FROM GENERIC
                    WHERE id = sibling_id;
                    ...
                    INSERT INTO GENERIC(tree_id,parent_id, id,lft,rgt)
                    VALUES (choosen_tree,choosen_parent_id,NULL, (lft_sibling + 1),(lft_sibling + 2));
    ...
    main:BEGIN
        ...
        ELSE
            BEGIN
                ...
                SELECT tree_id,parent_id,lft
                INTO choosen_tree,choosen_parent_id,rgt_sibling
                FROM GENERIC
                WHERE id = sibling_id;
                ...
                INSERT INTO GENERIC(tree_id,parent_id,id,lft,rgt)
                VALUES (choosen_tree,choosen_parent_id,NULL, rgt_sibling, rgt_sibling + 1);
...
DETERMINISTIC
    ...
    main:BEGIN
        ...
        DECLARE ref_parent_id INTEGER UNSIGNED;
        ...
        */
        SELECT tree_id, parent_id, IF(move_as_first_sibling,lft+1,lft), rgt
        INTO ref_node_tree, ref_parent_id, ref_lft, ref_rgt
        FROM GENERIC
        ...
        IF move_as_first_sibling = TRUE THEN
            UPDATE GENERIC
            SET parent_id = reference_node
            WHERE id=node_id_to_move;
        ELSE
            UPDATE GENERIC
            SET parent_id = ref_parent_id
            WHERE id=node_id_to_move;
        END IF;
